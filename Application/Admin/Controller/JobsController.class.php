...
class JobsController extends CommonController {
	...
	public function lists(){
		...
		$list = $Jobs->order('id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function post($type){
		...
		if($type=='add'){
			...
			if($_FILES['pic']['name']!=''){
				$pic = $Jobs->where('id='.I('id'))->getField('pic');
				@unlink('./Uploads/'.$pic);
			}

			$data['id'] = I('id');
			$result = $Jobs->save($data);
			if($result){
				$this->success('修改成功！');
			}else{
				$this->error('修改失败！');
	...
	public function delete(){
		...
		$pic = $Jobs->where('id='.I('id'))->getField('pic');
		@unlink('./Uploads/'.$pic);
		$result = $Jobs->delete(I('id'));
		if($result){
			$this->success('删除成功！',U('Jobs/lists'));
		}
	}
}
